...
for keyPlot in config:
    ...
    for iHisto in range(len(inputHistos)):
        ...
        histoDim   = 1 if "TGraph" in histoClass else inputHistos[iHisto].GetDimension()
        ...
        for iHisto in range(len(inputHistos)):
            ...
            if optionPlotter['verbRes']:
                ...
                if fitFunc:
                    ...
                    inputHistos[iHisto].Fit("gaus","0","",-4.,4.)
                    fitFunc = inputHistos[iHisto].GetFunction('gaus')
                    sigma    = fitFunc.GetParameter(2)
                    sigmaErr = fitFunc.GetParError(2)
                    legEntry = '{}  [gaussian core #sigma = {:4.2f}]'.format(legEntry, sigma)
